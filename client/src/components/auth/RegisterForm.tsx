...
export const RegisterForm: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setValidationErrors({});
    dispatch(register({ email, password, displayName: displayName || undefined }) as any);
  };

  return (
    <form onSubmit={handleSubmit} className="auth-form">
      <div className="auth-field">
        <label htmlFor="reg-displayName" className="auth-label">
          Display name (optional)
        </label>
        <input
          id="reg-displayName"
          type="text"
          className={`auth-input ${validationErrors.displayName ? 'auth-input--error' : ''}`}
          value={displayName}
          onChange={(e) => {
        ...
        {validationErrors.displayName && (
          <span className="auth-field-error">{validationErrors.displayName}</span>
        )}
      </div>
      <div className="auth-field">
      ...
      <div className="auth-field">
        ...
        <small className="auth-field-hint">
          At least 8 characters with one letter and one number (no special characters)
        </small>
      </div>
